...
class Result:
  ...
  members = ('args', 'argv', 'code', 'stdout', 'stderr', 'context')
  ...
  def format(self, fmt='yaml'):
    ...
    return getattr(formats, fmt).dumps({tag: self.as_dict()})
...
def run(argv, **kwargs):
  ...
  result.context.update({k: kwargs.get(k) for k in sorted(run.defaults.keys())})
  if result.context.stdout:
    result.context.stdout = repr(result.context.stdout)
  if result.context.stderr:
    result.context.stderr = repr(result.context.stderr)
